...
require "../../koneksi.php";
$nidn = $_SESSION["nidn"];
// ambil data bimbingan mahasiswas
$getbim = mysqli_query($conn, "SELECT * FROM pkl_bim WHERE nidn='$nidn' AND status='Bimbingan Laporan'") or die (mysqli_erorr($conn));
...
<head>
  ...
  <title>Data Bimbingan PKL | SIM-PS | Dosen</title>
      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <center>
              <h1 class="m-0 text-dark">Data Bimbingan Laporan PKL Belum Dibaca</h1>
            </center>
          </div>
          <!-- /.card-header -->
          <div class="card-body">
...
WHERE bim.nidn='$nidn'AND bim.status='Bimbingan Laporan'
...
if (mysqli_num_rows($getdata1) > 0) {
  while ($data1=mysqli_fetch_array($getdata1)) {
    ...
    ?>
                <tr>
                  <td><?= $data1['nim'] ?></td>
                  <td><a href="detailbimpkl.php?id=<?= $idbim?>">
                      <?= $data1['nama'] ?></a></td>
                  <td><?= $data1['judul']?></td>
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-12">
            <!-- <h1 class="m-0 text-dark">Data Bimbingan Laporan PKL</h1> -->
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </section>
    <div class="row">
      <div class="col-12">
        ...
        LEFT JOIN pkl_bim bim
        ON pkl.id_pkl=bim.id_pkl
        WHERE bim.nidn='$nidn'AND bim.status='Bimbingan Laporan' GROUP BY mhs.nama") 
        or die (mysqli_erorr($conn));
