...
$getbim = mysqli_query($conn, "SELECT * FROM proposal_bim WHERE pembimbing='$nidn' AND status='Bimbingan Proposal'") or die (mysqli_erorr($conn));
...
WHERE bim.pembimbing='$nidn'AND bim.status='Bimbingan Proposal' AND bim.nim='$decode' ORDER BY bim.tgl_bim ASC") or die (mysqli_erorr($conn));
...
<head>
  ...
  <title>Detail Bimbingan Proposal | SIM-PS</title>
      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <center>
              <h2 class="m-0 text-dark">Data Bimbingan Proposal,
                <?=$data1["nama"] ?></h2>
            </center>
          </div>
          <!-- /.card-header -->
