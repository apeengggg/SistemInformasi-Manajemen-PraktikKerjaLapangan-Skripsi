...
if ($w == 'Pembimbing 1') {
    ...
    windows.location.href('kp_validasisidpend.php')
    </script>";
    }else {
    echo "<script>
...
if ($w == 'Pembimbing 1') {
    ...
    $val = $_POST["valdosbing"];
    $pesan = $_POST["pesan"];
    $cs = mysqli_query($conn, "SELECT status_sidang FROM pend_sidang WHERE id_sidang='$id' AND (status_sidang='Lulus' OR status_sidang='Tidak Lulus')");
        if (mysqli_num_rows($cs)===1) {
        echo "<script>
        alert('Sidang Sudah Dilakukan, Tidak Dapat Mengubah Validasi')
        windows.location.href('kp_kp_validasisidpend.php')
        </script>";
        }else{
    $sqljadwal = mysqli_query($conn, "UPDATE pend_sidang SET
    val_dosbing1='$val',
...
<head>
  ...
  <title>Validasi Pendaftar Sidang Pendadaran | SIM-PS | Dosen</title>
...
<div class="content-wrapper">
      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <center>
              <h2 class="m-0 text-dark">Data Pendaftaran Sidang Pendadaran Mahasiswa Bimbingan</h2>
            </center>
          </div>
          <!-- /.card-header -->
          <div class="card-body">
            <table id="example1" class="table table-bordered table-striped">
              ...
              <tbody>
                <?php
                        ...
                        AND pp.status_penguji='Penguji 1' AND pp.status='Aktif'
                        LIMIT 0,1) AS penguji1,
                        -- penguji 2
                        (SELECT d1.nama_dosen FROM dosen d1 INNER JOIN pend_penguji pp
                        ON d1.nidn=pp.penguji WHERE pp.id_sidang=ps.id_sidang
                        AND pp.status_penguji='Penguji 2' AND pp.status='Aktif'
                        LIMIT 0,1) AS penguji2,
                        ps.tgl_sidang AS tgl,
                        ps.waktu_sidang AS waktu,
                        ps.ruang_sidang AS ruang, ps.id_sidang,
                        ps.val_dosbing1, ps.val_dosbing2,
                        s.id_skripsi
                        FROM pend_sidang ps
                        LEFT JOIN skripsi s
                        ...
                        WHERE (ps.val_dosbing1='0' OR ps.val_dosbing2='0')
                    ...
                    <span class="badge badge-primary">
                      ...
                      <span class="badge badge-success">
                        ...
                        <span class="badge badge-danger">
                          ...
                          <?= $val2  ?></td>
                  <td align="center">
                    <button type="submit" id="detaildata" class="btn-xs btn-warning" data-toggle="modal"
                      data-target="#modal-lg" data-idskripsi="<?= $data1["id_skripsi"] ?>"
                      data-id="<?= $data1["id_sidang"] ?>" data-nama="<?= $data1["nama"] ?>"
                      data-judul="<?= $data1["judul"] ?>" data-dosbing1="<?= $data1["pem1"] ?>"
                      data-dosbing2="<?= $data1["pem2"] ?>" data-tgl="<?= $data1["tgl"] ?>"
                      data-penguji1="<?= $data1["penguji1"] ?>" data-penguji2="<?= $data1["penguji2"] ?>"
            ...
            <table id="example3" class="table table-bordered table-striped">
              ...
              <tbody>
                <?php
                        ...
                        LEFT JOIN mahasiswa mhs
                        ON judul.nim=mhs.nim
                        WHERE ((ps.val_dosbing1='1' OR ps.val_dosbing2='1') OR (ps.val_dosbing1='2' OR ps.val_dosbing2='2'))
                        AND (sd.nidn='$nidn' AND sd.status_dosbing ='Pembimbing 1')
                        OR (sd.nidn='$nidn' AND sd.status_dosbing ='Pembimbing 2')") or die (mysqli_error($conn));
                    ...
                    <span class="badge badge-primary">
                      ...
                      <span class="badge badge-success">
                        ...
                        <span class="badge badge-danger">
                          ...
                          <?= $val2  ?></td>
                  <td align="center">
                    <button type="submit" id="detaildata1" class="btn-xs btn-warning" data-toggle="modal"
                      data-target="#modal-lg1" data-idskripsi="<?= $data2["id_skripsi"] ?>"
                      data-id="<?= $data2["id_sidang"] ?>" data-nama="<?= $data2["nama"] ?>"
                      data-judul="<?= $data2["judul"] ?>" data-dosbing1="<?= $data2["pem1"] ?>"
                      data-dosbing2="<?= $data2["pem2"] ?>" data-tgl="<?= $data2["tgl"] ?>"
                      data-penguji1="<?= $data2["penguji1"] ?>" data-penguji2="<?= $data2["penguji2"] ?>"
                      data-ruang="<?= $data2["ruang"] ?>" data-val1="<?= $val1 ?>" data-val2="<?= $val2 ?>"><i
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">Validasi Pendaftaran Sidang Pendadaran</h4>
          ...
          <form class="form-horizontal" method="post">
            <div class="card-body">
              <div class="form-group row">
                <div class="col-sm-9">
                  <input type="text" class="form-control" id="id_sidang" name="id_sidang" required="required" readonly
                    hidden>
                  <input type="text" class="form-control" id="id_skripsi" name="id_skripsi" required="required" readonly
                    hidden>
                </div>
              </div>
              <div class="form-group row">
              ...
              <div class="form-group row">
                ...
                <div class="col-sm-9">
                  <input type="text" class="form-control" id="id_dosenwali2" name="id_dosenwali2" required="required"
                    readonly>
                </div>
              </div>
              <div class="form-group row">
  ...
  <!-- modal ubah validasi sidang -->
  <div class="modal fade" id="modal-lg1">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">Ubah Validasi Pendaftaran Sidang Pendadaran</h4>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
