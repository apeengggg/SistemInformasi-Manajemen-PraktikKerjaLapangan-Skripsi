...
WHERE sd.nidn='$nidn' GROUP BY mhs.nim") 
...
if (isset($_POST["nilai1"])) {
  ...
  if ($w == 'Pembimbing 1') {
    ...
    $ap = $_POST["aplikasi"];
    // var_dump($_POST); die;
    $send = mysqli_query($conn, "UPDATE draft_nilai SET dos1_pen='$npm1', dos1_peng='$npm2', 
    dos1_sis='$sis', dos1_ap='$ap' WHERE id_sidang='$id'");
    if ($send) {
      echo "<script>
      alert('Nilai Berhasil Diinput !')
      windows.location.href='b_sidangdraft.php'
      </script>";
    }else {
      echo "<script>
    ...
    $ap = $_POST["aplikasi"];
    // var_dump($_POST); die;
    $send = mysqli_query($conn, "UPDATE draft_nilai SET dos2_pen='$npm1', dos2_peng='$npm2', 
...
LEFT JOIN dosen d
ON sd.nidn=d.nidn
WHERE sd.nidn='$nidn' AND ds.tgl_sidang='$date' ORDER BY ds.tgl_sidang DESC")
    ...
    <div class="row">
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <center>
              <h1 class="m-0 text-dark" id="penguji">Data Sidang Mahasiswa Bimbingan</h1><br>
            </center>
            <!-- <a href="#bim" class="btn btn-primary">Data Sidang Mahasiswa Bimbingan</a>
        ...
        while ($data1=mysqli_fetch_array($mhsbim) ) {
          ...
          ?>
                ...
                <tr>
                  ...
                  <td><a href="detailsidangdraft.php?id=<?= $id1 ?>"><?php echo $data1["nama"] ?></a></td>
