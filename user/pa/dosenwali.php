...
<head>
  ...
  <title>Data Dosen Wali | SIM-PS | Penasihat Akademik </title>
...
if (mysqli_num_rows($getdata) > 0) {
  while ($data=mysqli_fetch_array($getdata)) {
    ?>
                <tr>
                  ...
                  <!-- <td width="30px">
    <button type="submit" class="btn btn-sm btn-primary" id="detail"  data-toggle="modal" data-target="#modal"
    data-id="<?= $data['id_dosenwali'] ?>"
    data-nama="<?= $data['nama_dosen'] ?>">Ubah</button>
    </td> -->
                </tr>
                <?php  }
  }
  ...
  $get= mysqli_query($conn, "SELECT * FROM dosen"); ?>
                            <option disabled selected>Pilih Dosen Baru</option>
