...
echo "<script>
alert('Gagal Mengubah Memvalidasi Pendaftaran Sidang proposal')
windows.location.href('val_sidangprop.php')
</script>";
}
}
...
<head>
  ...
  <title>Validasi Pendaftaran Sidang Proposal | SIM-PS | Tata Usaha</title>
      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <center>
              <h2 class="m-0 text-dark">Data Pendaftaran Sidang Proposal</h2>
            </center>
          </div>
          <!-- /.card-header -->
          <div class="card-body">
            <table id="example1" class="table table-bordered table-striped">
              <thead align="center">
                <tr>
                  ...
                  <th width="120px">Validasi Pembimbing</th>
              ...
              <tbody>
                <?php
                        ...
                        "SELECT m.nim, m.nama, j.judul, ps.val_dosbing, pss.file
                        , pss.id_syarat, d.nama_dosen FROM proposal_sidang ps LEFT JOIN proposal p 
                        ON ps.id_proposal=p.id_proposal LEFT JOIN judul j 
                        ON p.id_judul=j.id_judul LEFT JOIN mahasiswa m 
                        ON j.nim=m.nim LEFT JOIN proposal_sidang_syarat pss
                        ON ps.id_sidang=pss.id_sidang LEFT JOIN dosen  d
                        ON p.dosbing=d.nidn
                        WHERE ps.val_dosbing='2' AND pss.status='0'")
                        or die (mysqli_erorr($conn));
                        while ($data1=mysqli_fetch_array($datasidang) ) {
                            if ($data1["val_dosbing"]==0) {
                                $val ='Menunggu';
                              }elseif ($data1["val_dosbing"]==1) {
                                $val ='Ditolak';
                              }elseif ($data1["val_dosbing"]==2) {
                                $val ='Disetujui';
                              }
                        ?>
                <!-- tampilkan data -->
                <tr>
                  ...
                  <td align="center"><a href="assets/downsidprop.php?filename=<?= $data1["file"]?>"
                  ...
                  <td align="center">
                    <button type="submit" id="detaildata" class="btn btn-warning" data-toggle="modal"
                      ...
                      data-judul="<?php echo $data1["judul"] ?>" data-dosbing="<?php echo $data1["nama_dosen"] ?>"><i
            ...
            <table id="example3" class="table table-bordered table-striped">
              ...
              <tbody>
                <?php
                        ...
                        "SELECT m.nama, m.nim, j.judul, d.nama_dosen, ps.id_sidang,
                        pss.id_syarat, pss.file, pss.status,
                        ps.val_dosbing
                        ...
                        LEFT JOIN dosen d 
                        ON p.dosbing=d.nidn
                        WHERE (pss.status='1' OR pss.status='2')")
                        or die (mysqli_erorr($conn));
                ...
                <tr>
                  ...
                  <td align="center"><a href="../../assets/syarat_sidang_proposal/<?= $data2["file"]?>"
                      class="btn btn-info"><i class="fas fa-download"></i></a></td>
                  <td align="center">
                    <button type="submit" id="detaildata1" class="btn btn-warning" data-toggle="modal"
                      ...
                      data-ids="<?php echo $data2["id_syarat"] ?>" data-nama="<?php echo $data2["nama"] ?>"
                      data-judul="<?php echo $data2["judul"] ?>" data-dosbing="<?php echo $data2["nama_dosen"] ?>"
                      data-status="<?php echo $status ?>"><i class="fas fa-edit"></i></button>
                  </td>
                </tr>
                <?php  }
                        ?>
