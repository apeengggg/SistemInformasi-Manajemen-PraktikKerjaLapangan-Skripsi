...
//ambi data bimbingan
$id_sid = mysqli_query($conn, "SELECT pn.id_sidang FROM pend_nilai pn INNER JOIN pend_sidang ps
ON pn.id_sidang=ps.id_sidang INNER JOIN skripsi s
...
$getdataa = mysqli_query($conn,
                        ...
                        ps.ruang_sidang AS ruang, ps.id_sidang AS id,
                        ps.status_sidang AS status, s.id_skripsi,
                        -- penguji 1
                        ...
                        AND ppe.status='Aktif' LIMIT 0,1) as penguji2,
                         -- ambil data nidn1
                         (SELECT d2.nidn FROM dosen d2 INNER JOIN pend_penguji ppe
                        ON d2.nidn=ppe.penguji 
                        WHERE ppe.id_sidang=ps.id_sidang AND status_penguji='Penguji 1'
                        AND ppe.status='Aktif' LIMIT 0,1) as nidn1,
                         -- ambil data nidn2
                         (SELECT d2.nidn FROM dosen d2 INNER JOIN pend_penguji ppe
                        ON d2.nidn=ppe.penguji 
                        WHERE ppe.id_sidang=ps.id_sidang AND status_penguji='Penguji 2'
                        AND ppe.status='Aktif' LIMIT 0,1) as nidn2,
                        ...
                        FROM pend_nilai WHERE id_sidang='$ida') AS total
                        FROM pend_sidang ps 
                        LEFT JOIN skripsi s
...
if (isset($_POST["ubahdata"])) {
  ...
  $ruang = $_POST["ruang"];
  $id = $_POST["id"];
  $ceksid = mysqli_query($conn, "SELECT * FROM pend_sidang WHERE id_sidang='$id' AND status_sidang IS NOT NULL");
    if (mysqli_num_rows($ceksid)>0) {
      echo "<script>
          alert('Sidang Sudah Dilaksanakan, Tidak Dapat Diubah')
          windows.location.href('detailsidangpend.php')
          </script>";
    }else{
...
if (mysqli_num_rows($cekjadwal)>0) {
    ...
    $sql3 = mysqli_query($conn, "UPDATE pend_sidang SET tgl_sidang='$tgl', 
  ...
  if (isset($_POST["ubahpeng"])) {
    ...
    if (mysqli_num_rows($ceksid)>0) {
      ...
      $cekpenguji = mysqli_query($conn, "SELECT penguji, tgl_sidang, waktu_sidang, ruang_sidang, ps.id_sidang 
                              FROM pend_sidang ps LEFT JOIN pend_penguji pp
                              ON ps.id_sidang=pp.id_sidang
                              WHERE (pp.penguji='$peng1' OR pp.penguji='$peng2') AND ps.id_sidang !='$id'
                              AND tgl_sidang='$tgl' AND waktu_sidang='$waktu' AND status_sidang IS NULL AND pp.status='Aktif'");
...
// var_dump($q); die;
    ...
    if ($peng1!=$ni1) {
      $nonaktif = mysqli_query($conn, "UPDATE pend_penguji SET status='Tidak Aktif'
                              WHERE penguji='$ni1' AND status_penguji='Penguji 1' AND id_sidang='$id'");
          if ($nonaktif) {
            $tambah = mysqli_query($conn, "INSERT INTO pend_penguji (id_sidang, penguji, status_penguji)
     ...
     if ($peng2!=$ni2) {
      $nonaktif2 = mysqli_query($conn, "UPDATE pend_penguji SET status='Tidak Aktif'
                               WHERE penguji='$ni2' AND status_penguji='Penguji 2' AND id_sidang='$id'");
        if ($nonaktif2) {
          $tambah1 = mysqli_query($conn, "INSERT INTO pend_penguji (id_sidang, penguji, status_penguji)
  ...
  if (isset($_POST["ubah1"])) {
    ...
    if ($statusq == "Tidak Lulus" || $statusq == "" ) {
      $ustatus = mysqli_query($conn, "UPDATE mahasiswa
                            ...
                            mahasiswa.status_skripsi ='-'
                            WHERE ps.id_sidang='$id'");
      $usid = mysqli_query($conn, "UPDATE pend_sidang SET status_sidang='Tidak Lulus' 
                                    WHERE id_sidang='$id'");
      $unilai = mysqli_query($conn, "UPDATE pend_nilai SET
...
<head>
  ...
  <title>Detail Sidang Pendadaran | SIM-PS | Tata Usaha</title>
    ...
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <ul class="navbar-nav">
            <li class="nav-item">
              <a href="sidangpend.php" class="btn btn-warning">Kembali</a>
      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <center>
              <h3 class="m-0 text-dark">Data Sidang Pendadaran,
                <?=$data1["nama"] ?></h3>
            </center>
          </div>
          <!-- /.card-header -->
          ...
          <div class="modal-dialog modal-lg">
            <div class="modal-content">
              <div class="modal-header">
                <h4 class="modal-title">Form Ubah Sidang Pendadaran</h4>
