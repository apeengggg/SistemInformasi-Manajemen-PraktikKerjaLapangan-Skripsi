...
$excel->getProperties()->setCreator('TU')
             ->setLastModifiedBy('TU')
             ->setTitle("Jadwal-S-PKL")
             ->setSubject("SidangPKL")
             ->setDescription("Jadwal-S-PKL")
             ->setKeywords("Sidang-PKL");
...
$excel->setActiveSheetIndex(0)->setCellValue('A1', "JADWAL PESERTA SIDANG PRAKTIK KERJA LAPANGAN"); // Set kolom A1 dengan tulisan "DATA SISWA"
...
$excel->setActiveSheetIndex(0)->setCellValue('H4', "PEMBIMBING"); // Set kolom F4 dengan tulisan "ALAMAT"
$excel->setActiveSheetIndex(0)->setCellValue('I4', "PENGUJI"); // Set kolom F4 dengan tulisan "ALAMAT"
...
$sql = mysqli_query($conn, "SELECT 
                        ...
                        JOIN mahasiswa
                        ON pkl.nim=mahasiswa.nim AND status_sid IS NULL ORDER BY tgl_sid ASC")
...
while($data = mysqli_fetch_array($sql)){ // Ambil semua data dari hasil eksekusi $sql
  ...
  $excel->setActiveSheetIndex(0)->setCellValue('B'.$numrow, $data['tgl_sid']);
  $excel->setActiveSheetIndex(0)->setCellValue('C'.$numrow, $data['waktu']);
  $excel->setActiveSheetIndex(0)->setCellValue('D'.$numrow, $data['ruang_sid']);
  $excel->setActiveSheetIndex(0)->setCellValue('E'.$numrow, $data['nim']);
  $excel->setActiveSheetIndex(0)->setCellValue('F'.$numrow, $data['nama']);
  $excel->setActiveSheetIndex(0)->setCellValue('G'.$numrow, $data['judul_laporan']);
  $excel->setActiveSheetIndex(0)->setCellValue('H'.$numrow, $data['pembimbing']);
  $excel->setActiveSheetIndex(0)->setCellValue('I'.$numrow, $data['penguji']);
...
$excel->getActiveSheet(0)->setTitle("JADWAL");
$excel->setActiveSheetIndex(0);

// Proses file excel
ob_end_clean();
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment; filename="Jadwal.xlsx"'); // Set nama file excel nya
