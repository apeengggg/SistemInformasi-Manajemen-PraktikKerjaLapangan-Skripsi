...
<head>
  ...
  <title>Validasi Pendaftaran Sidang Pendadaran | SIM-PS | Tata Usaha</title>
      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <center>
              <h2 class="m-0 text-dark">Data Pendaftaran Sidang Pendadaran</h2>
            </center>
          </div>
          <!-- /.card-header -->
          <div class="card-body">
...
"SELECT m.nim, m.nama, j.judul, ps.val_dosbing1, ps.val_dosbing2, pss.file
, pss.id_syarat, pss.status,
-- pem 1
...
ON ps.id_skripsi=s.id_skripsi LEFT JOIN proposal p
ON s.id_proposal=p.id_proposal
LEFT JOIN judul j ON p.id_judul=j.id_judul
LEFT JOIN mahasiswa m ON j.nim=m.nim
LEFT JOIN pend_sidang_syarat pss ON ps.id_sidang=pss.id_sidang
WHERE (ps.val_dosbing1='2' AND ps.val_dosbing2='2') AND pss.status='0'")
          ...
          ?>
            ...
            ?>
              ...
              ?>
                  ...
                  <td align="center"><a href="assets/downsidpend.php?filename=<?= $data1["file"]?>"
        ...
        <div class="card">
          ...
          <div class="card-body">
            <table id="example3" class="table table-bordered table-striped">
              ...
              LEFT JOIN mahasiswa m ON j.nim=m.nim
              LEFT JOIN pend_sidang_syarat pss ON ps.id_sidang=pss.id_sidang
              WHERE pss.status='2' OR pss.status='1'")
              or die (mysqli_erorr($conn));
              while ($data1=mysqli_fetch_array($datasidang) ) {
                if ($data1["status"]==0) {
                  $val ='Menunggu';
                ...
                if ($val=="Menunggu") {
                  ...
                  <td align="center"><a href="../../assets/syarat_sidang_pend/<?= $data1["file"]?>"
                      class="btn btn-info"><i class="fas fa-download"></i></a></td>
                  <td align="center">
                    <button type="submit" id="detaildata" class="btn btn-warning" data-toggle="modal"
                      data-target="#modal-lg" data-id="<?php echo $data1["id_sidang"] ?>"
